...
Zed = tf.Variable(tf.zeros_like(C, tf.float32))
...
# normalizing similar to described in https://www.ibm.com/developerworks/community/blogs/jfp/entry/My_Christmas_Gift
NormM = M - tf.log(tf.log(AbsZ))/np.log(2.0) + log_horizon

# normalizing similar to described in https://en.wikipedia.org/wiki/Mandelbrot_set#Continuous_.28smooth.29_coloring
# NormM = M + 1 - tf.log(tf.log(AbsZ))/np.log(2.0)

M = tf.where(not_diverged, Zed, NormM)
# M = tf.where(not_diverged, Zed, M)
...
fimage = fig.figimage(result, cmap='gnuplot2', norm=norm)
